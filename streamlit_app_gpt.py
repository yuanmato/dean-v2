"""
Ross Area & Faculty Toolkit — GPT/OpenAI Streamlit app (v5)
================================================
...
Deploy (Streamlit Community Cloud): put streamlit_app_gpt.py + ross_toolkit_gpt.py + requirements_gpt.txt
in a repo; add the secret OPENAI_API_KEY in the app dashboard; point the app at streamlit_app_gpt.py.
"""
...
with st.sidebar:
    ...
    model = st.selectbox("Model", [DEFAULT_MODEL, FAST_MODEL], index=0,
                         help="GPT-5.5 is the high-accuracy default; GPT-5.4 is the cheaper/faster fallback when available.")
...
def _verify_dept(client, s, d):
    ...
    with st.spinner(f"Re-checking {d['name']}…"):
        try:
            c = call_gpt_json(client, ss.meter, system=VERIFY_SYSTEM, user=user, model=model,
...
def page_faculty():
    ...
    if not ss.use_fetch:
        st.warning("Hosted web search is off — rosters will be much less current. Turn it on in the sidebar for best results.")
    disabled = client is None
    fr = ss.fac_results
