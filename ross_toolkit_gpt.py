#!/usr/bin/env python3
"""
Ross Area & Faculty Toolkit — GPT/OpenAI Python CLI / shared engine (v5)
=============================================================
...
Flags: --model, --concurrency, --no-web-search, --no-cache, --limit N, (faculty) --include-unmapped
"""
...
#  Config — model + tools                                                        #
# ----------------------------------------------------------------------------- #
DEFAULT_MODEL = "gpt-5.5"                 # best GPT model; change if your account uses a different id
FAST_MODEL    = "gpt-5.4"                 # cheaper/faster fallback when available

# OpenAI Responses API hosted web search tool. It can locate official directory pages and current web evidence.
WEB_SEARCH_TOOL = {"type": "web_search_preview", "search_context_size": "high"}

# Update these if your OpenAI account has different pricing; used only for the UI/CLI cost meter.
PRICE = {
...
VERIFY_SYSTEM = f"""You are DOUBLE-CHECKING the Ross-area mapping of ONE specific academic unit at ONE business school, because a human reviewer flagged it as possibly wrong. Re-investigate ONLY this unit. USE WEB SEARCH to inspect the unit's current official faculty pages; look at the actual faculty and the JOURNALS they publish in.
...
You are given the school, the list of departments already found, and the Ross areas still UNMAPPED. USE WEB SEARCH; judge by the journals faculty publish in. For EACH unmapped Ross area:
...
1. LOCATE the official faculty / people / directory page for THIS SPECIFIC department on the school's OWN website (an official .edu / school domain). If a likely URL is provided, start there. Otherwise web_search "<school> <department> faculty" or "<school> <department> faculty directory" and pick the school's OWN current listing — NEVER a third-party site, ranking, news article, Wikipedia, LinkedIn, ResearchGate, a faculty member's personal page, or an old/cached copy.

2. Use web search to inspect the official directory page and its visible current roster. Do not rely on your memory, old snippets, personal pages, Wikipedia, LinkedIn, Google Scholar, news articles, or third-party directories as the roster source. The people CURRENTLY listed on the official department/area directory as core ladder faculty ARE the roster.
...
3. EXHAUST EVERY PAGE — this is the #1 cause of missing faculty. Directories almost always show only a SLICE per page and split the rest across pagination, A–Z letters, rank tabs, or "load more" / "view all" controls. You MUST capture every slice:
   - URL-PARAMETER PAGINATION: if the page uses a query parameter such as page / p / pg / start / offset / from, iterate it (page=0, page=1, page=2, …) through web search/opening EACH official result page, following the directory's own "Next page" / numbered pagination links, until there are no more pages. The pagination control usually states the total number of pages — reach the LAST one. Do not stop at page 1.
   - Example pattern (a common Drupal directory): ".../directory?department=39&status=Active&page=0" then "&page=1", … — fetch each until the pagination runs out.
   - Also iterate any A–Z last-name tabs or rank filters the directory uses.
   Keep fetching until you have enumerated the COMPLETE list across all pages/tabs.
...
CORE / PRIMARY APPOINTMENT RULE (critical): count ONLY the department/area's CORE ladder faculty — people whose primary/home appointment is in THIS business-school department/area. EXCLUDE cross-listed, affiliated, courtesy, secondary, joint-from-another-school, or externally-primary faculty even if they have a Ross-style title or appear on a broad school-wide page. When in doubt, check the person's official profile: if the home unit/primary appointment is outside the target department/area, DROP them. Example calibration: for Michigan Ross Technology and Operations, David Mendez Emilien should NOT be counted as core T&O if his primary/home appointment is in Public Health rather than core Ross T&O.
...
9. Use web_search to (a) find the official directory URL, (b) reach additional official directory pages, or (c) confirm an individual's CURRENT rank/affiliation on official school pages. Search snippets alone are NOT the roster source. Always prefer the school's own current pages; distrust undated or old-looking sources.
...
def _response_text(resp):
    """Extract visible text from an OpenAI Responses API object across SDK versions."""
...
def _usage_counts(resp):
    ...
    # OpenAI may expose tool usage differently across SDK versions; keep the meter conservative.
    searches = 0
    return in_tok, out_tok, searches

def call_gpt_json(client, meter, *, system, user, model, use_web_fetch, max_tokens, tries=3):
    """Call OpenAI Responses API and parse the final JSON object.

    The parameter name `use_web_fetch` is kept for compatibility with the original app,
    but in this GPT version it means `use OpenAI hosted web search`.
    """
    tools = [WEB_SEARCH_TOOL] if use_web_fetch else []
    last = None
    for i in range(tries):
        ...
        in_tok, out_tok, searches = _usage_counts(resp)
        meter.add(in_tok, out_tok, searches)
        parsed = extract_json(text)
        if parsed is not None:
            return parsed
        raise ValueError("the model response did not contain a valid top-level JSON object; re-run this item")
    raise last

# Backward-compatible alias used by older imports/scripts.
call_gpt_json = call_gpt_json

# ----------------------------------------------------------------------------- #
#  Ross-area normalization# ----------------------------------------------------------------------------- #
...
def fetch_department_faculty(client, meter, model, use_fetch, s, d, *, recheck=False, prior=None,
    ...
    hint = f"Official directory page to start from (start from this official URL): {d['url']}\n" if d.get("url") else ""
    prior_block = ""
...
def stage1(args):
    ...
    print(f"Stage 1 — mapping {len(schools)} schools (each school's units -> Ross areas) "
          f"[model={args.model}, web_search={'on' if not args.no_web_search else 'off'}, conc={args.concurrency}]")
    results = {}
...
def stage2(args):
    ...
    def work(s, d):
        classify = bool(d["force_check"])
        key = f"facGPT5::{args.model}::{'C' if classify else 'S'}::{s['university']}||{s['school']}||{d['name']}"
        cached = cache.get(key)
        if cached is not None:
            return s, d, cached, True
        data = fetch_department_faculty(client, meter, args.model, not args.no_web_search, s, d)
        cache.put(key, data)
...
# ----------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Ross Area & Faculty Toolkit (v5 GPT, reversed mapping)")
    sub = ap.add_subparsers(dest="cmd", required=True)

    def common(p):
        p.add_argument("--model", default=DEFAULT_MODEL, help=f"default {DEFAULT_MODEL}; cheaper: {FAST_MODEL}")
    ...
    args = ap.parse_args()
    if OpenAI is None:
        sys.exit("Missing dependency. Run:  pip install -r requirements_gpt.txt")
